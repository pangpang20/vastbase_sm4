...
├── src/
│   ├── main/java/com/vastbase/hiveudf/
...
│   └── test/java/com/vastbase/hiveudf/
...
### 问题1: 函数注册失败
```
错误: ClassNotFoundException: com.vastbase.hiveudf.SM4EncryptECB
```
**解决**:
- 确认JAR已上传到HDFS
- 检查类名是否正确（包含完整包名）
...
2. **查看函数帮助**: `DESC FUNCTION EXTENDED sm4_encrypt_ecb;`
3. **运行测试**: `hive -f test_sm4_udf.hql`
