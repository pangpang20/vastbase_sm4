...
/*
 * sm4_encrypt_cbc_kdf(plaintext text, password text, hash_algo text) -> bytea
 * CBC模式加密，带密钥派生，返回salt+密文
 */
extern "C" Datum
sm4_encrypt_cbc_kdf(PG_FUNCTION_ARGS)
{
    text *plaintext = PG_GETARG_TEXT_PP(0);
    ...
    /* 分配输出缓冲区：salt(16) + 密文(最大填充后长度) */
    output_len = 16 + plain_len + SM4_BLOCK_SIZE;
    output = (uint8_t *)palloc(output_len);

    /* 加密 */
    if (sm4_cbc_encrypt_kdf((uint8_t *)pass_str, pass_len, hash_algo,
                            (uint8_t *)plain_str, plain_len,
                            output, &output_len) != 0) {
        pfree(plain_str);
        pfree(pass_str);
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("SM4 CBC KDF encryption failed")));
    }

    /* 构造bytea结果 */
    result = (bytea *)palloc(VARHDRSZ + output_len);
...
/*
 * sm4_decrypt_cbc_kdf(ciphertext bytea, password text, hash_algo text) -> text
 * CBC模式解密，带密钥派生，输入为salt+密文
 */
extern "C" Datum
sm4_decrypt_cbc_kdf(PG_FUNCTION_ARGS)
{
    bytea *ciphertext = PG_GETARG_BYTEA_PP(0);
    text *password = PG_GETARG_TEXT_PP(1);
    text *hash_algo_text = PG_GETARG_TEXT_PP(2);
    ...
    /* 解密 */
    if (sm4_cbc_decrypt_kdf((uint8_t *)pass_str, pass_len, hash_algo,
                            cipher, cipher_len,
                            plain, &plain_len) != 0) {
        pfree(pass_str);
        pfree(hash_algo);
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("SM4 CBC KDF decryption failed")));
    }
